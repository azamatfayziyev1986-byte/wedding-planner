...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const jwtSecret = process.env.JWT_SECRET || 'secret_key_timproduction_2026';
    ...
    res.status(500).json({ message: 'Server error: ' + (error.message || 'Xatolik') });
  }
});
...
// Server startup is handled inside startDatabase() above.
